...
$.code = $.isNode() ? (process.env.GUARDS_CODE ? process.env.GUARDS_CODE : 'H8mphLbwLg-qetebQoFihK8ouw2sT7YX'):'H8mphLbwLg-qetebQoFihK8ouw2sT7YX';
...
async function movement() {
  try {
		await takePostRequest('olympicgames_assist');
...
async function getPostBody(type) {
  return new Promise(async resolve => {
    ...
    try {
      ...
      if (type === 'olympicgames_assist') {
        taskBody = `functionId=${type}&body=${JSON.stringify({"inviteId": $.code,"type":"confirm","ss" : log})}&client=wh5&clientVersion=1.0.0&appid=${$.appid}`
      }
